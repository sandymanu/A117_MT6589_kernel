...
lbExit:
...
#if 0//LINE <SNTZT-58> <DATE20130711> <open zsd,continuous shot sometimes cannot stop> wupingzhou
	...
	else if ((eSensorType==SENSOR_TYPE_RAW) && (eSensorDev==SENSOR_DEV_MAIN)) {
		MY_LOGD(" wxb debug, createMtkZsdNccCamAdapter() ,zsd mode is NCC");
		return createMtkZsdNccCamAdapter(s8AppMode, i4OpenId, pParamsMgr); 
	}
	else
		return createMtkZsdCcCamAdapter(s8AppMode, i4OpenId, pParamsMgr);

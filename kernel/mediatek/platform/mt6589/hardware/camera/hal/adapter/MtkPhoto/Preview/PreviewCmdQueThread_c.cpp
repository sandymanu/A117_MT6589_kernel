...
#define LOG_TAG "MtkCamPhoto/PrvCQT"
...
typedef int (*Pr_Access_Beautifier_Initialize)(int maxFaceNum, int width, int height, int mode);
typedef void (*Pr_Access_Beautifier_Setting_Smartskin)(int ssRate);
typedef void (*Pr_Access_Beautifier_Setting_Whitening)(int whRate);
typedef int (*Pr_Access_Beautifier_Function_422)(unsigned char *in_y);
typedef void (*Pr_Access_Beautifier_Finalize)(void);

Pr_Access_Beautifier_Initialize pr_access_beautifier_initialize =NULL;
Pr_Access_Beautifier_Setting_Smartskin pr_access_beautifier_setting_smartskin =NULL;
Pr_Access_Beautifier_Setting_Whitening pr_access_beautifier_setting_whitening =NULL;
Pr_Access_Beautifier_Function_422 pr_access_beautifier_function_422 =NULL;
Pr_Access_Beautifier_Finalize pr_access_beautifier_finalize =NULL;
void *pr_handle = NULL;
#endif
...
#ifdef ACCESS_BEAUTIFIER_USE
		MY_LOGE("before smartskin whitening enable_access= %d\n", enable_access);
	    	if(pr_access_beautifier_setting_smartskin && enable_access)
			pr_access_beautifier_setting_smartskin(smartskin);
		if(pr_access_beautifier_setting_whitening && enable_access)
			pr_access_beautifier_setting_whitening(whitening);
		MY_LOGE("after smartskin:%d  whitening %d\n",smartskin , whitening);		
		if(pr_access_beautifier_function_422 && (whitening || smartskin) )
		{
			MY_LOGD("before pr_access_beautifier_function_422 \n");
	    		result_ps = pr_access_beautifier_function_422(inputImgBuf);
			MY_LOGD("after pr_access_beautifier_function_422 %d\n", result_ps);
		}
#endif
...
#ifdef ACCESS_BEAUTIFIER_USE
     String8 const s8LibPath = String8::format("/system/lib/libaccess_beautifier_c.so");
    if(pr_handle == NULL)
  {
	     ...
	     if ( !pr_handle )
	    {
	      ...
	      pr_access_beautifier_initialize = (Pr_Access_Beautifier_Initialize)::dlsym(pr_handle, "Access_Beautifier_initialize");
	      pr_access_beautifier_setting_smartskin = (Pr_Access_Beautifier_Setting_Smartskin)::dlsym(pr_handle, "Access_Beautifier_setting_smartskin");
	      pr_access_beautifier_setting_whitening = (Pr_Access_Beautifier_Setting_Whitening)::dlsym(pr_handle, "Access_Beautifier_setting_whitening");
	      pr_access_beautifier_function_422 = (Pr_Access_Beautifier_Function_422)::dlsym(pr_handle, "Access_Beautifier_function420P");
	      pr_access_beautifier_finalize = (Pr_Access_Beautifier_Finalize)::dlsym(pr_handle, "Access_Beautifier_finalize");
	     MY_LOGD("dlopen library pr_access_beautifier_initialize:%d \n", pr_access_beautifier_initialize);
...
start()
{
    ...
    MY_LOGE("Pass1Node.getImgBuf()->getImgWidth() == %d\n",Pass1Node.getImgBuf()->getImgWidth());
    MY_LOGE("Pass1Node.getImgBuf()->getImgHeight() == %d\n",Pass1Node.getImgBuf()->getImgHeight());
#if PREVIEW_TINNO_THREAD
...
stop()
{
	...
	if(pr_access_beautifier_finalize != NULL)
		pr_access_beautifier_finalize();
...
updateOne()
{
    	...
    	if(dispNode.getImgBuf() != 0)
	{
		...
		if(pr_access_beautifier_initialize != NULL)
			reset_cp = pr_access_beautifier_initialize(5, width, height, 1);

		MY_LOGE("after pr_access_beautifier_initialize\n");
#endif 
